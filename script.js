function data() {
  return {
    ...
    startTest() {
      // Clear any existing active timer to prevent memory leaks/stacking bugs
      if (this.interval) {
    ...
    calculateMetrics() {
      ...
      const wordCount = this.solution.trim() === "" ? 0 : this.solution.trim().split(/\s+/).length;
      const charCount = this.solution.length;

      this.words = wordCount;
      this.characters = charCount;
      this.wpm = Math.round((wordCount / this.time) * 60);
      this.cpm = Math.round((charCount / this.time) * 60);
    },

    checkFinished() {
      this.calculateMetrics();

      // Check if user has matched the full target string
